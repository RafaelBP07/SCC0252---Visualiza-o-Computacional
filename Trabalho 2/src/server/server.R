...
server <- function(input, output, session) {
  ...
  output$info_text_lineplot1 <- renderUI({
    tagList(
      tags$h4(tags$b("Contexto e Justificativa")),
      tags$p("Ao empregar gráficos de linha e heatmap em função do tempo na análise de dados do IMDB, buscamos desvendar padrões temporais que possam influenciar o desempenho dos filmes. A escolha dessas visualizações se justifica pela capacidade de capturar tendências ao longo do tempo e identificar correlações entre variáveis-chave, como faturamento, avaliações e participação do público. Sobre o Heatmap, ele é eficiente para representar intensidade temporal; saturação, como indicador visual de magnitude, intensifica a representação, possibilitando interpretação precisa das tendências."),
      tags$h4(tags$b("Interpretação do Gráfico")),
  ...
  output$info_text_heatmap1 <- renderUI({
    tagList(
      tags$h4(tags$b("Contexto e Justificativa")),
      tags$p("A aplicação de um heatmap ao longo dos anos, com a contagem de gêneros de filmes, visa proporcionar uma análise abrangente das tendências de produção cinematográfica ao longo do tempo. A escolha dessa visualização é motivada pela busca por padrões nas preferências do público em relação aos gêneros cinematográficos. O heatmap, ao mapear visualmente a distribuição das contagens de gêneros ao longo dos anos, oferece insights cruciais sobre a evolução dos gostos do público e orienta decisões estratégicas na indústria cinematográfica. Também os Heatmaps são particularmente eficazes para representar a intensidade de fenômenos ao longo do tempo, sendo que o brilho, neste contexto, atua como um indicador visual da magnitude dessas relações. A utilização desse canal de cor intensifica a representação das informações, permitindo uma interpretação mais precisa e diferenciada das tendências observadas."),
      tags$h4(tags$b("Interpretação do Gráfico")),
